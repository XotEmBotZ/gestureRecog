...
void save_buffer_to_nvs(const char* label, float* buffer, int num_channels, int buffer_size) {
  ...
  err = nvs_set_blob(my_handle, label, buffer, data_size);
  if (err == ESP_OK) {
    err = nvs_commit(my_handle);
    if (err == ESP_OK) {
      ESP_LOGI(TAG, "Successfully stored normalized buffer with label: %s", label);
    }
  } else {
...
void list_stored_buffers(int num_channels, int buffer_size) {
  ...
  while (it != NULL) {
    ...
    if (info.type == NVS_TYPE_BLOB) {
      ...
      if (required_size > 0) {
        float* data = malloc(required_size);
        if (data) {
          if (nvs_get_blob(my_handle, info.key, data, &required_size) == ESP_OK) {
            int num_floats = required_size / sizeof(float);
            for (int ch = 0; ch < num_channels; ch++) {
              printf("  CH%d: ", ch);
              for (int i = 0; i < buffer_size; i++) {
                int val_idx = ch * buffer_size + i;
                if (val_idx < num_floats) {
                  printf("%.2f%s", data[val_idx], (i == buffer_size - 1) ? "" : ",");
                }
              }
              printf("\n");
            }
